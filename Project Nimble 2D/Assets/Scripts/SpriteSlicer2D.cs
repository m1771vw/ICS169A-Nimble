...
#region SLICING_METHODS
	...
	public static void ExplodeSprite(GameObject sprite, int numCuts, float explosionForce, bool destroySlicedObjects, ref List<SpriteSlicer2DSliceInfo> slicedObjectInfo)
	{
		...
		if(GetSpriteBounds(sprite, out spriteBounds))
		{
			...
			for(int loop = 0; loop < numCuts; loop++)
			{
				...
				Vector3 jitterOffset = new Vector3(Mathf.Sin(randomAngle) * (spriteBounds.size.x * 0.25f), Mathf.Cos(randomAngle) * (spriteBounds.size.y * 0.25f), 0.0f);
	...
	public static void ShatterSprite(GameObject spriteObject, float explosionForce, bool destroySlicedObjects, ref List<SpriteSlicer2DSliceInfo> slicedObjectInfo)
	{
		...
		if(polygonPoints != null && polygonPoints.Count > 3)
		{
			...
			int[] triangles = Triangulate(ref polygonPoints);
			...
			for(int loop = 0; loop < triangles.Length; loop += 3)
			{
				...
				Vector2[] childSpriteVertices = points.ToArray();;
				...
				if(parenttk2dSprite)
				{
					childSprite.InitFromTK2DSprite(parenttk2dSprite, ref childCollider, ref childSpriteVertices);
				}
				else
#endif
				...
				Vector2 centrePosition = parentCentre + ((childSpriteVertices[0] + childSpriteVertices[1] + childSpriteVertices[2]) * 0.33f);
			...
			for(int cutResultIndex = 0; cutResultIndex < cutStartResults.Length && cutResultIndex < cutEndResults.Length; cutResultIndex++)
			{
				...
				if(cutEnter.rigidbody == cutExit.rigidbody)
				{
					...
					if(polygonPoints != null)
					{
						...
						if(!s_AllowConvexSlicing && !IsConvex(ref polygonPoints))
						{
						...
						if(!AreVerticesAcceptable(ref childSprite1Vertices, child1Area) || !AreVerticesAcceptable(ref childSprite2Vertices, child2Area))
						{
							continue;
						}
						else
						{
							...
							if(parenttk2dSprite)
							{
								childSprite1.InitFromTK2DSprite(parenttk2dSprite, ref child1Collider, ref childSprite1Vertices);
								childSprite2.InitFromTK2DSprite(parenttk2dSprite, ref child2Collider, ref childSprite2Vertices);
							}
							else
#endif
							if(parentSlicedSprite)
							{
								childSprite1.InitFromSlicedSprite(parentSlicedSprite, ref child1Collider, ref childSprite1Vertices);
								childSprite2.InitFromSlicedSprite(parentSlicedSprite, ref child2Collider, ref childSprite2Vertices);
							}
							else if(parentUnitySprite)
							{
								childSprite1.InitFromUnitySprite(parentUnitySprite, ref child1Collider, ref childSprite1Vertices);
								childSprite2.InitFromUnitySprite(parentUnitySprite, ref child2Collider, ref childSprite2Vertices);
							}
	...
	static int[] Triangulate(ref List<Vector2> points) 
	{
	...
	/// 
	static bool AreVerticesAcceptable(ref Vector2[] vertices, float area)
	{
		...
		if(!s_AllowConvexSlicing && !IsConvex(ref vertices))
		{
			if(s_DebugLoggingEnabled)
			{
...
public class SlicedSprite : MonoBehaviour
{
	...
	/// </summary>
	public void InitFromTK2DSprite(tk2dSprite sprite, ref PolygonCollider2D polygon, ref Vector2[] polygonPoints)
	{
		...
		InitSprite(sprite.gameObject, sprite.GetComponent<MeshRenderer>(), ref polygon, ref polygonPoints, spriteDefinition.uvs[0], spriteDefinition.uvs[spriteDefinition.uvs.Length - 1], spriteDefinition.GetBounds(), spriteDefinition.material, isRotated, isHFlipped, isVFlipped, Vector2.zero, Vector2.zero);
    ...
    /// </summary>
    public void InitFromSlicedSprite(SlicedSprite slicedSprite, ref PolygonCollider2D polygon, ref Vector2[] polygonPoints)
    {
        InitSprite(slicedSprite.gameObject, slicedSprite.MeshRenderer, ref polygon, ref polygonPoints, slicedSprite.MinCoords, slicedSprite.MaxCoords, slicedSprite.SpriteBounds, slicedSprite.m_MeshRenderer.sharedMaterial, slicedSprite.Rotated, slicedSprite.HFlipped, slicedSprite.VFlipped, slicedSprite.m_Centroid, slicedSprite.m_UVOffset);
    ...
    /// </summary>
    public void InitFromUnitySprite(SpriteRenderer unitySprite, ref PolygonCollider2D polygon, ref Vector2[] polygonPoints)
    {
        ...
        {
            if (s_MaterialList[loop].mainTexture.GetInstanceID() == spriteTexture.GetInstanceID())
            {
        ...
        InitSprite(unitySprite.gameObject, unitySprite.GetComponent<Renderer>(), ref polygon, ref polygonPoints, minTextureCoords, maxTextureCoords, unitySprite.sprite.bounds, material, false, false, false, Vector2.zero, pivotVector);
    ...
    /// </summary>
	void InitSprite(GameObject parentObject, Renderer parentRenderer, ref PolygonCollider2D polygon, ref Vector2[] polygonPoints, Vector3 minCoords, Vector3 maxCoords, Bounds spriteBounds, Material material, bool rotated, bool hFlipped, bool vFlipped, Vector2 parentCentroid, Vector2 uvOffset)
    {
